    ...
    <div className="flex flex-col h-full py-6 px-4">
      ...
      <nav className="flex-1 space-y-1">
        {NAV.map(n => {
          ...
          return (
            <Link
              ...
              href={n.href}
              onClick={onClose}
              className={`flex items-center gap-3 px-4 py-2.5 rounded-full text-sm font-medium transition-colors whitespace-nowrap ${
                active
                  ? 'bg-blue-50 text-[#0F61E9]'
                  : 'text-gray-700 hover:bg-gray-100'
              }`}
            >
              <span className={`flex-shrink-0 ${active ? 'text-[#0F61E9]' : 'text-gray-500'}`}>{n.icon}</span>
              {n.label}
            </Link>
          );
        })}
        <button
          ...
          className="w-full flex items-center gap-3 px-4 py-2.5 rounded-full text-sm font-medium text-gray-700 hover:bg-gray-100 transition-colors"
        >
