...
// Comprehensive country list with dial codes and emoji flags
...
export function PhoneDialCodeSelect({ value, onChange }: Props) {
  const [open, setOpen] = useState(false);
  const [search, setSearch] = useState('');
  const ref = useRef<HTMLDivElement>(null);
  const searchRef = useRef<HTMLInputElement>(null);

  const selected = COUNTRIES.find((c) => c.dial === value) ?? COUNTRIES.find((c) => c.code === 'GB')!;
          ...
          <ul className="dial-list">
            {filtered.map((c) => (
              <li key={c.code}>
                <button
                  type="button"
                  className={`dial-item ${c.dial === value ? 'active' : ''}`}
                  onClick={() => {
                    onChange(c.dial);
                    setOpen(false);
                    setSearch('');
            ...
            {filtered.length === 0 && (
              <li className="dial-empty">No results</li>
            )}
          </ul>
        </div>
      )}
